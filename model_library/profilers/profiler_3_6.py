...
# ------------
# Based on profiler_3_5
# bigger initial calibrate_factor
# ------------
...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        self.calibrate_factor = tf.Variable(20.0, name='calibrate_factor')
    ...
    def calculate_from_profile(self, pred_profile):
        ...
        arcsin = tf.math.asin(tf.math.divide_no_nan(34.0, calculated_Vmax))

        finite_value = tf.where(tf.math.is_finite(arcsin), arcsin, 0.0)

        calibration = self.calibrate_factor * tf.stop_gradient(finite_value)

        calculated_R34 = over_R34_count * 5.0 + calibration
