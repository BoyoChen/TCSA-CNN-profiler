...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        # conv layers
        self.conv1 = layers.Conv2D(16, kernel_size=(8, 3), strides=[2, 2], padding='same')
        self.conv2 = layers.Conv2D(32, kernel_size=(8, 3), strides=[2, 2], padding='same')
        self.conv3 = layers.Conv2D(64, kernel_size=(8, 3), strides=[2, 2], padding='same')
        self.conv4 = layers.Conv2D(128, kernel_size=(8, 3), strides=[2, 2], padding='same')
        self.conv5 = layers.Conv2D(256, kernel_size=(8, 3), strides=[2, 2], padding='same')
        self.conv6 = layers.Conv2D(512, kernel_size=(8, 3), strides=[2, 2], padding='same')
    ...
    def calculate_from_profile(self, pred_profile):
        ...
        calculated_R34 = tf.math.reduce_max(
            tf.cast(pred_profile >= 34.0, tf.float32) * tf.range(0, 751, 5, dtype=tf.float32),
            axis=1
        )
